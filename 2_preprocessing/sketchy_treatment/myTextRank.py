...
stopwords = readLines('D:/nnu/大创/weibo_tour_sentiment_dict/2_PreProcessing/predict/stop_words.txt')
...
def extract_keywords(i):
    df = pd.read_csv('D:/nnu/大创/weibo_tour_sentiment_dict/2_PreProcessing/data/数据汇总.csv', index_col=0, )
    data = []
    d = []
    for x in df['微博正文']:
...
if __name__ == "__main__":
    ...
    file = open('D:/nnu/大创/weibo_tour_sentiment_dict/2_PreProcessing/data/热门带权值.txt', 'a+', encoding='utf-8')
    for i in range(200):
        file.write(str(pairs[i]) + '\n')
    file.close()
    file = open('D:/nnu/大创/weibo_tour_sentiment_dict/2_PreProcessing/data/热门.txt', 'a+', encoding='utf-8')
    for i in range(200):
